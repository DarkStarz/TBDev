...
$r1 = mysql_query('select name, id, description, minclassview, forid, sort FROM overforums ORDER BY sort') or  sqlerr(__FILE__,__LINE__);
...
$htmlout .= stdmsg('Err','There are no overforums, maybe you should add some');
	...
	if($do =='edit' && $id) {
		$r3 = mysql_query('select name, id, description , minclassview ,forid, sort FROM overforums WHERE id ='.$id) or sqlerr(__FILE__,__LINE__);
