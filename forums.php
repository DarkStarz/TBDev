...
//==Begin cached online users
	...
	echo "<p align='center'>(+) next to your username indicates you are Anonymous !</p>";
	}
 	echo "{$forumusers}</td>
 	</tr>
 	<tr>
   ...
   echo "&nbsp;<font class='small'>[<a class='altlink' href='".$_SERVER['PHP_SELF']."?action=editforum&amp;forumid=".$forumid."'>Edit</a>][<a class='altlink' href='".$_SERVER['PHP_SELF']."?action=deleteforum&amp;forumid=".$forumid."'>Delete</a>]</font>";
     ...
     echo "<br />". htmlspecialchars($forums_arr["description"]);
        ...
        if ($show_mods == true && isset($mods_array[$forumid]))
           echo "<br/>" . showMods($mods_array[$forumid]);
...
// -------- Inserts a compose frame
   ...
   // $htmlout='';
    if ($newtopic) {
        ...
        echo "<h3>New topic in <a href='". $_SERVER['PHP_SELF']."?action=viewforum&amp;forumid=".$id."'>".htmlspecialchars($arr["name"])."</a> forum</h3>";
        } else {
   ...
   echo "
    ...
    echo begin_frame("Compose", true);
    echo "<form method='post' name='compose' action='".$_SERVER['PHP_SELF']."' enctype='multipart/form-data'>
    ...
    $htmlout .="<tr>
		...
		{
		echo "<textarea name='body' style='width:99%' rows='7'>{$qbody}</textarea>";
		}
		echo "</td></tr>";
      ...
      if ($newtopic){
      echo "Anonymous Topic<input type='checkbox' name='anonymous' value='yes'/>\n";
      }
      else
      {
      echo "Anonymous Post<input type='checkbox' name='anonymous' value='yes'/>\n";
      }
      echo "</td></tr>\n";
    ...
    if (!$newtopic) {
        ...
        if (mysql_num_rows($postres) > 0) {
           ...
           echo "<br />";
           echo begin_frame("10 last posts, in reverse order");
            ...
            while ($post = mysql_fetch_assoc($postres)) {
             ...
             echo "<p class='sub'>#" . $post["id"] . " by <i>Anonymous</i> at ".get_date($post["added"], 'LONG',1,0)."</p>";
             }
             else{	
             echo "<p class='sub'>#" . $post["id"] . " by <i>Anonymous</i> (<b>" . $post["username"] . "</b>) at ".get_date($post["added"], 'LONG',1,0)."</p>"; 
             }
             else
             echo "<p class='sub'>#".$post["id"]." by ". (!empty($post["username"]) ? $post["username"] : "unknown[{$post['uid']}]")." at ".get_date($post["added"], 'LONG',1,0)."</p>";
...
if ($action == 'updatetopic') {
    ...
    if (isMod($topic_arr["forumid"]) || $CURUSER['class'] >= UC_MODERATOR) {
        ...
        echo begin_frame("Edit Forum", "center");
        ...
        echo "<option value='$i' " . ($i == $forum['minclassread'] ? " selected='selected'" : "") . ">" . get_user_class_name($i) . "</option>\n";
        echo "</select> Class required to View<br />\n&nbsp;Minimum <select name='writeclass'>";
        for ($i = 0; $i <= MAX_CLASS; ++$i)
        echo "<option value='$i' " . ($i == $forum['minclasswrite'] ? " selected='selected'" : "") . ">" . get_user_class_name($i) . "</option>\n";
        echo "</select> Class required to Post<br />\n&nbsp;Minimum <select name='createclass'>";
        for ($i = 0; $i <= MAX_CLASS; ++$i)
        echo "<option value='$i' " . ($i == $forum['minclasscreate'] ? " selected='selected'" : "") . ">" . get_user_class_name($i) . "</option>\n";
	...
	if ($use_poll_mod && is_valid_id($pollid))
	{
		...
		if (mysql_num_rows($res) > 0)
		{
		  ...
		  $arr1["option15"], $arr1["option16"], $arr1["option17"], $arr1["option18"], $arr1["option19"]);
			...
			echo "<table cellpadding='5' width='{$forum_width}' align='center'>
			<tr><td class='colhead' align='left'><h2>Poll";
			if ($userid == $t_userid || $CURUSER['class'] >= UC_MODERATOR)
			{
			echo "<font class='small'> - [<a href='".$_SERVER['PHP_SELF']."?action=makepoll&amp;subaction=edit&amp;pollid=".$pollid."'><b>Edit</b></a>]</font>";
			if ($CURUSER['class'] >= UC_MODERATOR)
			{
			echo "<font class='small'> - [<a href='".$_SERVER['PHP_SELF']."?action=deletepoll&amp;pollid=".$pollid."'><b>Delete</b></a>]</font>";
			}
			}
			echo "</h2></td></tr>";

			echo "<tr><td align='center' class='clearalt7'>";
			echo "
			<table width='55%'>
      ...
      for($i=0;$i<20;$i++) $vs[$i]=0;
         ...
         foreach($os as $a) 
				{
					...
					$c = ($i % 2 ? '' : "poll");
					echo "<tr>";
	        echo "<td width='1%' style='padding:3px;white-space:nowrap;' class='embedded".$c."'>".htmlspecialchars($a[1])."</td>";
					echo "<td width='99%' class='embedded".$c."' align='center'>";
					echo "<img src='{$TBDEV['pic_base_url']}bar_left.gif' alt='bar_left.gif' />
				  ...
				  echo "</table>";
				  echo "<p align='center'>Votes: <b>".number_format($tvotes)."</b></p>";
			    }
		    	else
			    {
				  echo "<form method='post' action='".$_SERVER['PHP_SELF']."?action=viewtopic&amp;topicid=".$topicid."'>
          ...
          $voters .= ', ';
 	        ...
 	        $voters = "<i>Anonymous</i>(<a href='{$TBDEV['baseurl']}/userdetails.php?id=".(int)$arr4['userid']."'><b>".$arr4['username']."</b></a>)";
				  ...
				  echo $voters."<br />(<font class='small'><a href='".$_SERVER['PHP_SELF']."?action=viewtopic&amp;topicid=$topicid'>hide</a></font>)";
				  }
			    }
		      echo "</td></tr></table>";
		    }
       ...
       echo "<br /><a href='{$TBDEV['baseurl']}/subscriptions.php?topicid=$topicid&amp;subscribe=1'><b><font color='red'>Subscribe to Forum</font></b></a>";
       echo "<br /><br />";
...
/*]]>*/
    ...
    while ($arr = mysql_fetch_assoc($res)) {
        ...
        echo "<tr><td width='737' colspan='2'><table class='main'><tr><td style='border:none;' width='100%'>{$posticon}<a  id='p".$postid."' name='p{$postid}' href='".$_SERVER['PHP_SELF']."?action=viewtopic&amp;topicid=".$topicid."&amp;page=p".$postid."#p".$postid."'>#".$postid."</a> by ".$by." at ".$added."";
       
        if (isset($newp)) {
          echo "$newp";
        }
       
        echo "</td><td style='border:none;'><a href='#top'><img align='right' src='{$TBDEV['pic_base_url']}".$forum_pics['arrow_up']."' alt='Top' /></a></td></tr></table></td></tr>";
      ...
      echo "<tr align='center'><td width='150' align='center' style='padding: 0px'>";
      if ($arr["p_anon"] == "yes") {
      if($CURUSER['class'] < UC_MODERATOR && $posterid != $CURUSER["id"])
      echo "<img width='150' src='pic/default_avatar.gif' alt='Avatar' /><br />";
      else
      echo "<img width='150' src='{$avatar}' alt='Avatar' /><br />
	  ...
	  echo "</td><td class='text' width='100%'>{$body}</td></tr><tr><td>";
		  if ($arr["p_anon"] == "yes") {
 	    if($CURUSER['class'] < UC_MODERATOR)
		  echo "";
		  else
 	      echo "<img src='".$TBDEV['pic_base_url'].$forum_pics[($last_access > (time()-360) || $posterid == $CURUSER['id'] ? 'on' : 'off').'line_btn']."' border='0' alt='' />&nbsp;<a href='{$TBDEV['baseurl']}/sendmessage.php?receiver=".$posterid."'><img src='".$TBDEV['pic_base_url'].$forum_pics['pm_btn']."' border='0' alt='Pm ".htmlspecialchars($postername)."' /></a>";
		  }
      else 
      {
      echo "<img src='".$TBDEV['pic_base_url'].$forum_pics[($last_access > (time()-360) || $posterid == $CURUSER['id'] ? 'on' : 'off').'line_btn']."' border='0' alt='' />&nbsp;<a href='{$TBDEV['baseurl']}/sendmessage.php?receiver=".$posterid."'><img src='".$TBDEV['pic_base_url'].$forum_pics['pm_btn']."' border='0' alt='Pm ".htmlspecialchars($postername)."' /></a>";
      ...
      echo "<a href='{$TBDEV['baseurl']}/report.php?type=Post&amp;id=".$postid."&amp;id_2=".$topicid."&amp;id_3=".$posterid."'><img src='".$TBDEV['pic_base_url'].$forum_pics['p_report_btn']."' border='0' alt='Report Post' /></a>";
      ...
      echo "&nbsp;&nbsp;&nbsp;<a href='{$TBDEV['baseurl']}/usermood.php' onclick=\"return popitup('usermood.php')\">
      <span class='tool'>
      ...
      <span class='tip'>".$moodanon."&nbsp;".htmlspecialchars($mooduname)."&nbsp;!</span></span></a></td>";
        ...
        if (!$locked || $CURUSER['class'] >= UC_MODERATOR || isMod($forumid)) {
 	      ...
 	      echo "<a href='".$_SERVER['PHP_SELF']."?action=quotepost&amp;topicid=".$topicid."&amp;postid=".$postid."'><img src='".$TBDEV['pic_base_url'].$forum_pics['p_quote_btn']."' border='0' alt='Quote Post' /></a>"; 
 			  }
        else 
        {
	      echo "<a href='".$_SERVER['PHP_SELF']."?action=quotepost&amp;topicid=".$topicid."&amp;postid=".$postid."'><img src='".$TBDEV['pic_base_url'].$forum_pics['p_quote_btn']."' border='0' alt='Quote Post' /></a>"; 
        ...
        echo "<a href='".$_SERVER['PHP_SELF']."?action=deletepost&amp;postid=".$postid."'><img src='".$TBDEV['pic_base_url'].$forum_pics['p_delete_btn']."' border='0' alt='Delete Post' /></a>";
        }

        if (($CURUSER["id"] == $posterid && !$locked) || $CURUSER['class'] >= UC_MODERATOR || isMod($forumid)) {
        echo "<a href='".$_SERVER['PHP_SELF']."?action=editpost&amp;postid=".$postid."'><img src='".$TBDEV['pic_base_url'].$forum_pics['p_edit_btn']."' border='0' alt='Edit Post' /></a>";
        }
    ...
    if ($use_poll_mod && (($userid == $t_userid || $CURUSER['class'] >= UC_MODERATOR || isMod($forumid)) && !is_valid_id($pollid))) {
		...
		echo "<table cellpadding='5' width='{$forum_width}'>
        <tr>
        	<td align='right'>
   ...
   $maypost = ($CURUSER['class'] >= $arr["write"] && $CURUSER['class'] >= $arr["create"]);
    if ($locked && $CURUSER['class'] < UC_MODERATOR && !isMod($forumid)) {
      ...
      echo "<p align='center'>This topic is locked; no new posts are allowed.</p>";
       } else {
        $arr = get_forum_access_levels($forumid);

        if ($CURUSER['class'] < $arr["write"]) {

          echo "<p align='center'><i>You are not permitted to post in this forum.</i></p>";
	...
	echo "<table align='center' class='main' border='0' cellspacing='0' cellpadding='0'><tr>
	<td class='embedded'>
	...
	if ($maypost)
	{
	echo "<td class='embedded' style='padding-left: 10px'>
	<form method='get' action='".$_SERVER['PHP_SELF']."'>
    ...
    if ($locked)
		...
		{
		echo "<table style='border:1px solid #000000;' align='center'><tr>
  ...
  if (!empty($uploaderror))
	...
	echo "<form name='compose' method='post' action='".$_SERVER['PHP_SELF']."?action=editpost&amp;postid=".$postid."'>
    ...
    if (function_exists('textbbcode'))
    echo textbbcode("compose", "body", $ebody);
    else {
    echo "<textarea name='body' style='width:99%' rows='7'>{$ebody}</textarea>";
    }
    
		echo "</td></tr>";
	  if ($CURUSER["class"] >= UC_MODERATOR)
    echo "<tr><td colspan='1' align='center'><input type='checkbox' name='lasteditedby' /></td><td align='left' colspan='1'>Don't show the Last edited by <font class='small'>(Staff Only)</font></td></tr>";
	 echo "<tr>
  ...
  <input type='hidden' name='action' value='".$action."' />
	...
	if ($subaction == "edit")
	{
	echo "<input type='hidden' name='pollid' value='".(int)$poll["id"]."'>";
	}
	echo "
...
else if ($use_attachment_mod && $action == "whodownloaded")
{
  ...
  $dls = 0;
	...
	echo "<tr align='center'>".
	  ...
	  $dls += (int)$arr['downloads'];
		...
		echo "<tr><td colspan='4'><b>Total Downloads:</b><b>".number_format($dls)."</b></td></tr></table></body></html>";
	}
	//print($HTMLOUT);
}
    ...
    if ($subforums > 0) {
	  echo "<table border='1' cellspacing='0' cellpadding='5' width='{$forum_width}'>
    ...
    <td  align='right'>Topics</td>
        ...
        echo show_forums($forumid, true);
        echo end_table();
    }

    if (mysql_num_rows($topics_res) > 0) {
    echo "<br /><table border='1' cellspacing='0' cellpadding='5' width='{$forum_width}'>
		<tr>
	...
	if (!$maypost)
	{
	echo "<p><i>You are not permitted to start new topics in this forum.</i></p>";
	}
	echo "<table border='0' class='main' cellspacing='0' cellpadding='0' align='center'>
	<tr>
	...
	exit();
        ...
        if ((isset($_POST[$action]) ? $_POST[$action] : '') == 'clear') {
			...
			echo "<form method='post' action='{$TBDEV['baseurl']}/forums.php?action=viewunread'>
  ...
  else if ($action == "getdaily")
  {
	...
	echo "<h2 align='center'>Today Posts (Last 24 Hours)</h2>";
	echo $pager['pagertop'];
	...
	echo "<tr>
      ...
      <td align='center'>";
				...
				{
				echo "<a href='{$TBDEV['baseurl']}/userdetails.php?id=".$posterid."'>".htmlspecialchars($getdaily['username'])."</a>";
				}
				else
				{
				echo "<b>unknown[".$posterid."]</b>";
				}
			  echo "</td>";
		
	      echo "<td style='white-space: nowrap;'>".get_date($getdaily['added'], 'LONG',1,0)."</td></tr>";
...
else if ($action == "search") //-------- Action: Search
{
  ...
  <font class='small'><b>Note:</b> Searches <u>only</u> in posts.</font></td>
	...
	echo "<h1 align='center'><b><a href='".$_SERVER['PHP_SELF']."'>Forums</a></b> -> ". htmlspecialchars($arr["name"])."</h1>
